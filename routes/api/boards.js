    ...
    async (req, res, next) => {
        ...
        if (board.type === 'Public') {
            ...
            if (req.header('x-auth-token') !== 'null') {
                req.board = board;
                next();
            } else notAuthorizeBoard(res, board, 'This WhiteBoard is protected by a password')
        }
    }, auth,
    async (req, res) => {
        try {
            console.log("kbhjbhbubyubyuy iubiyubuy \n\n\n\n\n\n\n\n\n\n\n\n",req);
            if (req.user.id == req.board.owner) authorizeBoard(res, req.board, { role: 'OWNER' });
            else notAuthorizeBoard(res, req.board, 'This WhiteBoard is protected by a password');
        }
        catch (err) {
            console.log("err\n\n\n\n\n\n\n\n\n\n\n\n",req);
            notOk(err, res);
